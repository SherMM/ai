...
def get_adjacent(grid, i, j):
    '''
    Returns adjacent whitespace pixels
    where whitespace is represented by a 0
    in the pixel grid
    '''
...
def getWhitespaceComponents(grid, n, m):
    '''
    Returns list of whitespace connected
    component sizes
    '''
    # discovered conpoments
    components = []
    seen = set()
    ...
    def process(node):
        ...
        while queue:
            ...
            seen.add((i, j))
            on_queue.remove((i, j))
            for adj in get_adjacent(grid, i, j):
                if adj not in seen:
                    if adj not in on_queue:
                        queue.append(adj)
                        on_queue.add(adj)
...
def enhancedFeatureExtractorDigit(datum):
    ...
    ## DESCRIBE YOUR ENHANCED FEATURES HERE...
        ...
        The enhanced features for digit identification
        take advantage of the fact that different digits
        have different numbers of whitespace connected 
        components.
        
        A function that finds the number of connected 
        components in an undirected graph represented 
        as a grid using Breadth First Search was used 
        to find the connected components and return a 
        list of their sized. 

        After examining the number of connected
        components returned by the getWhiteSpaceComponents()
        function, it was determine that 1 to 4 components 
        are returned. So, binary features for these types
        of occurences were initialized. This can be seen below
    ##
    """
    features =  basicFeatureExtractorDigit(datum)
    pixels = datum.getPixels()
    
    # use BFS to find connected components of 
    # whitespace to differentiate digits
    comps = getWhitespaceComponents(pixels, len(pixels), len(pixels[0]))
    # set up binary features
    features['has-1-comp'] = 1 if len(comps) == 1 else 0
    features['has-2-comp'] = 1 if len(comps) == 2 else 0
    features['has-3-comp'] = 1 if len(comps) == 3 else 0
    features['has-4-comp'] = 1 if len(comps) == 4 else 0
    return features
...
def enhancedPacmanFeatures(state, action):
    ...
    Basically this just uses the code from my betterEvaluationFuction() from
    project 2, but instead of returning a score, sets the evaluation score
    as a feature
    """
    features = util.Counter()
    
    # we need to get the next state of the game, given the parameter action
    nextState = state.generatePacmanSuccessor(action)
    
    # use my evaluation function from Project 2 in multiAgents.py
    numFood = nextState.getNumFood()
    foodList = nextState.getFood().asList()
    pacmanPosition = nextState.getPacmanPosition()
    newGhostStates = nextState.getGhostStates()
    newScaredTimes = [ghostState.scaredTimer for ghostState in newGhostStates]
    eatScore = nextState.getScore()
    
    # handle walls and add a score for it
    # play with the constant
    walls = sum([nextState.hasWall(x, y) for x,y in getSurroundingCoords(pacmanPosition)])
    features['walls'] = 0.0001 * walls

    # handle goals of game: eating food and avoiding becoming food
    if foodList:
        ghostList = nextState.getGhostPositions()
        closeFoodDist, _ = min([(util.manhattanDistance(pacmanPosition, p), p) for p in foodList])
        closeGhostDist, _ = min([(util.manhattanDistance(pacmanPosition, p), p) for p in ghostList])
        
        # account for ghost being near pacman
        if closeGhostDist < 3:
            # no scared ghosts are near, so pacman should be fearful
            if sum(newScaredTimes) == 0:
                features['score'] = 0
            else:
                features['score'] = 10*(1.0/closeFoodDist) + eatScore + (1.0/numFood) + 10*closeGhostDist
        else:
            features['score'] = 10*(1.0/closeFoodDist) + eatScore + (1.0/numFood) + 0.1*closeGhostDist
    else:
        features['score'] = eatScore
    return features


def getSurroundingCoords(pacmanPosition):
    """
    Returns pacman's surrounding coordinates
    """
    x, y = pacmanPosition
    pos = [
