...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        tempBeliefs = util.Counter() # temporary belief distribution
        
        # check if noisyDistance is None, which 
        # indicates that a ghost has been captured and 
        # should be placed in jail position
        if noisyDistance is not None:
            # update probabilities at all valid locations
            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                likelihood = emissionModel[trueDistance]
                prior = self.beliefs[p]
                if likelihood > 0.0:
                    # calculate & assign the new probability
                    # for belief at that position
                    tempBeliefs[p] = likelihood * prior
        else:
            # indicate the ghost has been captured
            # and placed in jail
            jailPosition = self.getJailPosition()
            tempBeliefs[jailPosition] = 1.0
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # loop through current(old) ghost positions
        for currPos in self.legalPositions:
            # get prob dist for possible positions ghost could move to
            # from current position. Have to get temporary game state to
            # explore this without mutation as comments above describe
            tempGameState = self.setGhostPosition(gameState, currPos)
            newPosDist = self.getPositionDistribution(tempGameState)
            for newPos, prob in newPosDist.items():
                if prob > 0.0:
                    # update beliefs based on new position prob dist
                    tempBeliefs[newPos] += self.beliefs[currPos] * prob
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        # need to cycle through list of legal positions
        # until we have numParticles items, ensuring that
        # they are evenly distributed by continually re-looping
        # through the list of legal positions
        count = 0
        self.beliefs = []
        for position in itertools.cycle(self.legalPositions):
            if count >= self.numParticles:
                break
            else:
                self.beliefs.append(position)
                count += 1
    ...
    def observe(self, observation, gameState):
        ...
        tempBeliefs = util.Counter() # temp belief distribution
        
        if noisyDistance is None:
            # signals that ghost has been captured so
            # need to set ghost jail positions for all particles
            self.beliefs = [self.getJailPosition()] * self.numParticles
        else:
            for currPos in self.beliefs:
                # update beliefs
                trueDistance = util.manhattanDistance(pacmanPosition, currPos)
                tempBeliefs[currPos] += emissionModel[trueDistance]

            if not all(val == 0 for val in tempBeliefs.values()):
                # get a new sample of particles
                dist, vals = tempBeliefs.values(), tempBeliefs.keys()
                self.beliefs = util.nSample(dist, vals, self.numParticles)
            else:
                # need to re-initialize since all particle weights are zero
                self.initializeUniformly(gameState)
    ...
    def elapseTime(self, gameState):
        ...
        a belief distribution.
        """
        tempBeliefs = [] # list of temporary belief positions
        for currPos in self.beliefs:
            # get prob dist for possible positions ghost could move to
            # from current position. Have to get temporary game state to
            # explore this without mutation as comments above describe
            tempGameState = self.setGhostPosition(gameState, currPos)
            posDist = self.getPositionDistribution(tempGameState)          
            tempBeliefs.extend(util.nSample(posDist.values(), posDist.keys(), 1))
        self.beliefs = tempBeliefs
    ...
    def getBeliefDistribution(self):
        ...
        Counter object)
        """
        # we want to count particles at a position, and divide by
        # the total number of particles to get the probability dist
        beliefDist = util.Counter()
        beliefDist.incrementAll(self.beliefs, 1)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        weight with each position) is incorrect and may produce errors.
        """
        count, r = 0, self.numGhosts
        self.beliefs = []
        # get permutations of possible ghost positions
        perms = list(itertools.product(self.legalPositions, repeat=r))
        # loop through position tuples until we have numParticles of them
        for posGroup in itertools.cycle(perms):
            if count >= self.numParticles:
                break
            else:
                self.beliefs.append(posGroup)
                count += 1
    ...
    def observeState(self, gameState):
        ...
        # if the noisyDistance at position i in the noisyDistances 
        # list is None, then the ghost has been captured and we put it in jail
        if any(val is None for val in noisyDistances):
            self.putGhostsInJail(noisyDistances)
        
        for belief in self.beliefs:
            weights = []
            for i, value in enumerate(noisyDistances):
                # check if None which would indicate ghost is captured
                # code will also fail without this check
                if value is not None:
                    currPos = belief[i]
                    dist = util.manhattanDistance(pacmanPosition, currPos)
                    weights.append(emissionModels[i][dist])
            # multiplies weights together, or sets to 1
            product = reduce(mul, weights) if weights else 1
            # sums the multiplied weights together
            tempBeliefs[belief] += product

        if not all(val == 0 for val in tempBeliefs.values()):
            # need to sample particles 
            tempBeliefs.normalize()
            dist, vals = tempBeliefs.values(), tempBeliefs.keys()
            self.beliefs = util.nSample(dist, vals, self.numParticles)
        else:
            # all weights are zero, so we need to reinitialize
            self.initializeParticles()
            # check and put ghosts in jail if necessary after init
            if any(val is None for val in noisyDistances):
                self.putGhostsInJail(noisyDistances)
    ...
    def putGhostsInJail(self, noisyDistances):
        '''
        Loops through ghosts, checks if their noisy distance is
        None. If so, places that ghost in jail
        '''
        for i, value in enumerate(noisyDistances):
            if value is None:
                for j, belief in enumerate(self.beliefs):
                    self.beliefs[j] = self.getParticleWithGhostInJail(belief, i)        
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.beliefs:
            ...
            particles = []
            for i in range(self.numGhosts):
                tempState = setGhostPositions(gameState, newParticle)
                # get ghost pos distr at new game state
                newPosDist = getPositionDistributionForGhost(tempState, i, self.ghostAgents[i])
                dist, vals = newPosDist.values(), newPosDist.keys()
                # sample a particle and add to list
                particles.extend(util.nSample(dist, vals, 1))
